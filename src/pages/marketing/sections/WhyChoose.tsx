    ...
    <section className="relative px-5 py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          <Card>
            <MiniSyncFlow />
            <h3 className="mt-5 text-lg font-bold text-slate-900">Everything Syncs Automatically</h3>
          ...
          <Card>
            <MiniAppGrid />
            <h3 className="mt-5 text-lg font-bold text-slate-900">One Platform, Every Module</h3>
            <p className="mt-2 text-sm leading-relaxed text-slate-500">
          ...
          <Card>
            <MiniAutomationStack />
            <h3 className="mt-5 text-lg font-bold text-slate-900">Automate the Busywork</h3>
            <p className="mt-2 text-sm leading-relaxed text-slate-500">
...
function MiniSyncFlow() {
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/60 px-4 py-5">
      {steps.map((s, i) => (
        <div key={s} className="flex items-center gap-2">
          <span className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 shadow-sm">
            {s}
          </span>
          {i < steps.length - 1 && <ArrowRight className="size-3.5 text-indigo-400" />}
        </div>
      ))}
    </div>
  )
...
function MiniAppGrid() {
  const apps = ['🧾', '🛒', '👥', '📦', '🧑‍💼', '📊']
  return (
    <div className="grid grid-cols-3 gap-2 rounded-xl border border-slate-100 bg-slate-50/60 p-4">
      {apps.map((a, i) => (
        <div
          key={i}
          className="grid aspect-square place-items-center rounded-lg border border-slate-200 bg-white text-lg shadow-sm"
        >
          {a}
        </div>
      ))}
...
function MiniAutomationStack() {
  ...
  return (
    <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50/60 p-4">
      {items.map((t) => (
        <div key={t} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-sm">
