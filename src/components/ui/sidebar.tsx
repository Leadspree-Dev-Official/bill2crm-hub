...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<"div"> & { showIcon?: boolean }) {
  // Derived from the component's own id rather than Math.random(): an impure
  // call in the render body can produce a different width on every pass under
  // concurrent rendering, making the skeleton visibly jitter while loading.
  const id = React.useId()
  const width = `${50 + (Array.from(id).reduce((a, c) => a + c.charCodeAt(0), 0) % 40)}%`
